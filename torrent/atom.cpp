...
CAtomList :: CAtomList( const CAtomList &c )
{
	...
	vector<CAtom *> vecList = c.getValue( );

	for( vector<CAtom *> :: iterator i = vecList.begin( ); i != vecList.end( ); i++ )
	{
...
vector<CAtom *> *CAtomList :: getValuePtr( )
{
	return &m_vecList;
}
...
CAtomDicti :: CAtomDicti( const CAtomDicti &c )
{
	...
	map<string, CAtom *> mapDicti = c.getValue( );
...
map<string, CAtom *> CAtomDicti :: getValue( ) const
{
	return m_mapDicti;
}

map<string, CAtom *> *CAtomDicti :: getValuePtr( )
{
	return &m_mapDicti;
}
